...
Say we want to publish these changes. Let's `push` them to the remote:

```{bash, error = TRUE}
git push
```
Git will complain that we don't have an upstream branch. This means that it 
doesn't know which remote branch to update. Thankfully it at least
provides instructions!

The `-u` flag to `git push` is shorthand for `--set-upstream`.
...
(yes, it is probably confusing to use the term `upstream` to refer to both
an "upstream branch" on any remote, and a specific remote - it's been worse!)
...
You can also edit remotes --- useful if for example, you cloned `git@github.com:/qupath/qupath.git`
to quickly get some changes, but you don't want to push some random edits to
the main qupath repo...
```{bash}
